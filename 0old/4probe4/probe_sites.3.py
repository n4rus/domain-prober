...
def probe_domains(candidates, phase: str, batch_size: int, max_workers: int,
    ...
    # If there is a checkpoint, load it and set resume to True.
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for candidate in candidates:
            # Skip until last candidate is found (this is the resuming logic).
            if resume:
                if candidate == last_candidate:
                    resume = False  # Start processing after the last processed candidate
                continue
